    ...
    class Base(private val api: Api) : Repository {
        override fun fetch(body: String, callback: DataCallback) {
            api.fetch(Api.RequestBody.Base(body), object : Api.Callback {
                override fun provideSuccess(data: Api.Result.Success) {
                    callback.provideSuccess(data.data)
                }

                override fun provideError(e: Api.Result.Error) {
                    callback.provideError(e.e.message ?: "error")
                }
            })
        }
    }
